...
// checks if arguments are made before running code.
if (args.length < 1) {
    console.log(`Please enter valid users`);
} else {
    ...
    // checks if the users are valid in users list
    if (usersArrayById.length < 1) {
        console.log(`Please enter valid users`);
    } else {
        // console.log(`usersArrayById: ${usersArrayById}`);
        ...
        // console.log(`allEventsFromInputUsers: ${allEventsFromInputUsers}`)
        ...
        for (let date = startDate; date <= endDate; date++) {
            eventsFilteredByUserAndDate.push(
                allEventsFromInputUsers.filter((event) => event.start_time.getUTCDate() == date)
            );
        }

        // console.log(`eventsFilteredByUserAndDate: ${eventsFilteredByUserAndDate}`);
        ...
        eventsFilteredByUserAndDate.map((event) => {
            let startTimeOfTheDay = new Date(
                Date.UTC(year, monthIndex, event[0].start_time.getUTCDate(), startOfDayTime)
            );
